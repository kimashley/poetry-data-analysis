...
sonnet_or_free_verse <- c(z, z, y, y)
...
# change model
# data analysis 
# human is 0, gpt2 is 1
# sonnet is 0, free verse is 1

poems.data <- poems.data[!is.na(poems.data$ratings),]
poems.data$ratings <- as.ordered(as.factor(poems.data$ratings))

# formula1 <- ratings ~ question * (human_or_machine + sonnet_or_free_verse + human_or_machine:sonnet_or_free_verse) + (1|people) + (1|poem)
# formula1 <- ratings1 ~ question * human_or_machine + sonnet_or_free_verse + human_or_machine:sonnet_or_free_verse + (1|people) + (1|poem) 
#model <- brm(formula = formula1, 
             #data = poems.data, family = "cratio")
#model
#predict(model)

#install.packages("brmstools")

#posterior_predict(model, resp = )
